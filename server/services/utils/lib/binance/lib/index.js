...
let api = function Binance(options = {}) {
  ...
  const COOLDOWN_TIME = 60 * 1000; // 60 giây
  Binance.cooldownUntil = 0;
  if (options) setOptions(options);

  async function waitForCooldown() {
    const now = Date.now();
    if (Binance.cooldownUntil > now) {
      const waitTime = Binance.cooldownUntil - now;
      await new Promise((res) => setTimeout(res, waitTime));
      Binance.cooldownUntil = 0;
      Binance.info.isBanned = false;
    }
  }
  ...
  const marginOrder = (
    ...
    signedRequest(
      ...
      function (error, response) {
        ...
        if (callback) callback(error ? error.body : error, response);
        else
          Binance.options.log(
            side + "(" + symbol + "," + quantity + "," + price + ") ",
            response
  ...
  const futuresOrder = async (
    ...
    if (
      !params.timeInForce &&
      (params.type === "STOP" || params.type === "TAKE_PROFIT")
    ) {
      params.timeInForce = "GTX"; // Post only by default. Use GTC for limit orders.
    } else if (params.type === "LIMIT") params.timeInForce = "GTC";
  ...
  const checkError = (err) => {
    if (
      (err && err.code === -1003) ||
      (err && err.body && err.body.code === -1003)
    ) {
      Binance.info.isBanned = true;
      // Extract timestamp from error message if available
      const banUntilMatch =
        err.msg?.match(/banned until (\d+)/) ||
        err.body?.msg?.match(/banned until (\d+)/);

      if (banUntilMatch) {
        Binance.cooldownUntil = parseInt(banUntilMatch[1]);
        console.error(
          `[Binance] IP banned until ${new Date(
            Binance.cooldownUntil
          ).toISOString()}`
        );
      } else {
        Binance.cooldownUntil = Date.now() + COOLDOWN_TIME;
        console.error("[Binance] Hit rate limit, cooldown 60s");
      }
    }
  };
  const promiseRequest = async (url, data = {}, flags = {}) => {
    if (Binance.info.isBanned || Binance.info.usedWeight > 2200) {
  ...
  return {
    ...
    futuresChangeMultiAssetsMode: async (multiAssetsMode, params = {}) => {
      params.multiAssetsMargin = multiAssetsMode;
    ...
    getSubAccount: async (symbol, params = {}) => {
      // Either orderId or origClientOrderId must be sent
      params.symbol = symbol;
      return promiseRequest("v1/sub-account/list", params, {
        base: sapi,
        type: "SIGNED",
      });
    },
    getSubAccountAssets: async (email, params = {}) => {
      // Either orderId or origClientOrderId must be sent
      params.email = email;
      return promiseRequest("v1/sub-account/futures/account", params, {
        base: sapi,
        type: "SIGNED",
      });
    },
    // transferSubAccount: async (symbol, amount, fromEmail, toEmail, params = {}) => {
    //     params.asset = symbol;
    //     params.amount = amount;
    //     params.fromEmail = fromEmail;
    //     params.toEmail = toEmail;
    //     params.futuresType = 1;
    //     return promiseRequest("v1/sub-account/futures/internalTransfer", params, {
    //         base: sapi,
    //         method: "POST",
    //         type: "SIGNED",
    //     });
    // },
    transferFuturesSubAccount: async (
      symbol,
      amount,
      fromEmail,
      params = {}
      ...
      params.type = 2;
      return promiseRequest("v1/sub-account/futures/transfer", params, {
        base: sapi,
        method: "POST",
        type: "SIGNED",
      });
    },
    transferSubAccount: async (
      symbol = "USDT",
      amount,
      fromEmail,
      toEmail,
      ...
      params.amount = amount;
      params.fromEmail = fromEmail;
      params.fromAccountType = "USDT_FUTURE";
      params.toAccountType = "SPOT";
      params.toEmail = toEmail;
      //  params.futuresType = 1;
      return promiseRequest("v1/sub-account/universalTransfer", params, {
        base: sapi,
        method: "POST",
        type: "SIGNED",
      });
    ...
    mgAccount: function (callback, isIsolated = false) {
      const endpoint =
        "v1/margin" + (isIsolated ? "/isolated" : "") + "/account";
